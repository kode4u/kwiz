H5P.Column = (function (EventDispatcher) {
  ...
  function Column(params, id, data) {
    ...
    self.attach = function ($container) {
      ...
      setTimeout(function() { initRiseScrollObserver(wrapper); }, 80);
    };
  ...
  /**
   * Scroll Reveal Observer for Rise Animated Experience
   */
  
  /**
   * Universal Mobile & Desktop Scroll Reveal Controller (Handles Full Height Iframes & Touch Scroll)
   */
  function initRiseScrollObserver(containerEl) {
    if (!containerEl) return;
    
    var elements = Array.prototype.slice.call(
      containerEl.querySelectorAll(
        ".h5p-column-content, .rise-step-item, .rise-gallery-item, .rise-header-block, .rise-stepper-container, .rise-video-card, .rise-gallery-card, .rise-summary-block, .h5p-accordion, .h5p-multichoice"
      )
    );
    ...
    elements.forEach(function (el) {
      el.classList.add("rise-scroll-animate");
    });
    ...
    function checkVisibility() {
      ...
      var parentTopOffset = 0;

      // Handle Moodle Parent Page Scroll when iframe height is expanded
      try {
        if (window.parent && window.parent !== window && window.frameElement) {
          var iframeRect = window.frameElement.getBoundingClientRect();
          parentTopOffset = iframeRect.top;
      ...
      var triggerLine = viewportHeight * 0.94;

      for (var i = pendingElements.length - 1; i >= 0; i--) {
        var el = pendingElements[i];
        var rect = el.getBoundingClientRect();
        var elementScreenTop = rect.top + parentTopOffset;

        // If top of element is near or inside viewport
        if (elementScreenTop <= triggerLine) {
          el.classList.add("rise-revealed");
    ...
    // Listen to scroll and touch on current iframe window
    window.addEventListener("scroll", triggerOnScroll, { passive: true });
    window.addEventListener("touchmove", triggerOnScroll, { passive: true });
    window.addEventListener("resize", triggerOnScroll, { passive: true });

    // Listen to scroll and touch on parent Moodle page
    try {
      if (window.parent && window.parent !== window) {
        window.parent.addEventListener("scroll", triggerOnScroll, { passive: true });
        window.parent.addEventListener("touchmove", triggerOnScroll, { passive: true });
        window.parent.addEventListener("resize", triggerOnScroll, { passive: true });
      }
    } catch (e) {}
  }
