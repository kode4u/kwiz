...
H5P.ImageSlider = (function ($) {
  ...
  C.prototype.attach = function ($container) {
    ...
    var sliderStyle = this.options.sliderStyle || 'rise-cinema';
    $container
      .addClass("h5p-image-slider")
      .addClass('h5p-image-slider-using-mouse')
      .addClass('h5p-theme')
      .addClass('slider-style-' + sliderStyle);
  ...
  C.prototype.attachControls = function() {
    ...
    C.handleButtonClick(this.$leftButton, function (e) {
      if (e) { e.preventDefault(); e.stopPropagation(); }
      if (!self.dragging) {
        if (self.currentSlideId > 0) {
          self.gotoSlide(self.currentSlideId - 1);
        } else {
          self.gotoSlide(self.imageSlides.length - 1);
        }
      }
    });

    C.handleButtonClick(this.$rightButton, function(e) {
      if (e) { e.preventDefault(); e.stopPropagation(); }
      if (!self.dragging) {
        if (self.currentSlideId < self.imageSlides.length - 1) {
          self.gotoSlide(self.currentSlideId + 1);
        } else {
          self.gotoSlide(0);
        }
  ...
  C.prototype.updateNavButtons = function() {
    ...
    this.$leftButton.show().css('opacity', '1');
    this.$rightButton.show().css('opacity', '1');
  };
