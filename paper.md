...
Our evaluation reveals an average local LLM generation time of 8.11–12.42 seconds per question using `qwen2.5-coder:7b` with instant **0.0 ms** embedding cache hits. An expert pedagogical review of 125 generated questions across five programming domains yields a 96.0% overall acceptability rating, with zero context hallucinations and high inter-rater agreement (Cohen’s $\kappa = 0.88$). System Usability Scale (SUS) scores from instructors indicate excellent usability (mean score = 82.5), validating KwizRAG as a sustainable, cost-effective, and classroom-ready solution for digital assessment.
